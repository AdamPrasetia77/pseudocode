/* ====== Konfigurasi link Quizizz (ganti dengan link kelasmu) ====== */
const QUIZIZZ = {
  pretest: "https://quizizz.com/join",
  posttest: "https://quizizz.com/join",
};
document.getElementById("pretestBtn").href = QUIZIZZ.pretest;
document.getElementById("posttestBtn").href = QUIZIZZ.posttest;
...
function run() {
  ...
  try {
    ...
    body = body.slice(0, -1);
    exec(parseBlock(body, 0, body.length), vars, out, { steps: 0 });
    output.textContent = out.length ? out.join("\n") : "(program selesai tanpa keluaran)";
